...
class Jaiminho_Signup_Shortcode{

	static function init(){
		add_shortcode('jaiminho-signup', array('Jaiminho_Signup_Shortcode','load_form'));
	}
	    ...
	    <div class="sendpress-signup-form">
			<form id="sendpress_signup" method="POST" <?php if( !$widget_options['load_ajax'] ){ ?>class="sendpress-signup"<?php } else { ?>action="?sendpress=post"<?php } ?> >
				...
				<div id="form-wrap" <?php if( $sendpress_show_thanks ){ echo 'style="display:none;"'; } ?>>
					...
					<?php if( filter_var($display_job, FILTER_VALIDATE_BOOLEAN) ): ?>
						<p name="job">
							<?php if( !$label ): ?>
								<label for="job"><?php echo $job_label; ?>:</label>
							<?php endif; ?>
							<input type="text" class="sp_lastname" orig="<?php echo $job_label; ?>" value="<?php if($label){ echo $job_label; } ?>" name="sp_job" />
						</p>
					<?php endif; ?>
